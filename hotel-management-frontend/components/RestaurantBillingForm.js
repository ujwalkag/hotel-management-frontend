...
import printDetailedBill from "@/utils/printDetailedBill"
...
function RestaurantBillingForm() {
  ...
  const handleGenerateBill = async () => {
    ...
    const payload = {
      customer_name: (customerInfo.name || "").trim() || "Guest",
    ...
    try {
      ...
      toast.success(`✅ D-mart Style Bill Generated! Receipt: ${result.receipt_number}`);

      await printDetailedBill(result, billingSettings.tableNumber, result.items, true);
